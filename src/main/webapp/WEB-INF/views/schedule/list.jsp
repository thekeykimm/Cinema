		...
		<table
			...
			<c:forEach var="list" items="${scheduleList}">
				<tr>
					...
					<td><a href="hView?schNum=${list.schNum}">${list.schNum}</a></td>
					<td>${list.movTitle}</td>
					<td>${list.cinName}</td>
					<td>${list.scrNum}</td>
					<td>${list.schDate}</td>
...
<script>
	$('#limit').change(()=>{
		...
		location.href = "hList?limit=" + limit;
	});
